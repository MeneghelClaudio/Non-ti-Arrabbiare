...
def _adj(color, d):
    """Sposta i componenti RGB di delta."""
...
def _grad_v(surf, rect, r, ct, cb, at, ab):
    """Gradiente verticale liscio dentro rect con angoli arrotondati."""
...
def _panel(surf, rect, radius, c_top=(25, 25, 32), c_bot=(40, 40, 52),
           a_top=215, a_bot=200, border_color=None, border_w=1):
    """Pannello scuro con gradiente e angoli arrotondati."""
    tmp = pygame.Surface((rect.width, rect.height), pygame.SRCALPHA)
    _grad_v(tmp, pygame.Rect(0, 0, rect.width, rect.height), 0,
...
def _player_dot(surf, cx, cy, r, color):
    """Pallino colorato con riflesso."""
    pygame.draw.circle(surf, color, (cx, cy), r)
    pygame.draw.circle(surf, _adj(color, +70), (cx - r // 3, cy - r // 3), max(r // 3, 1))
...
class PlayerNames:
    """Gestisce i nomi dei giocatori. Legge da players.json o usa nomi automatici."""
    ...
    def set_names(self, names_list):
        """Aggiorna i nomi (chiamato dal sistema di login)."""
...
class TurnBanner:
    ...
    def draw(self, screen, current_player, phase_name, player_name, sw):
        ...
        _grad_v(glass, pygame.Rect(1, 1, w - 2, h // 2 - 1), r,
...
class DiceDisplay:
    """Mostra il risultato del dado con animazione bounce."""
    ...
    def draw(self, screen, dice_roll, sw):
        ...
        _panel(surf, pygame.Rect(0, 0, rw, rh), r,
        ...
        _grad_v(glass, pygame.Rect(1, 1, rw - 2, rh // 2 - 2), r,
...
class Leaderboard:
    """Classifica laterale con pedine al traguardo e in casa."""
    ...
    def draw(self, screen, players, player_names, current_player, sh):
        ...
        for i, player in enumerate(ranked):
            ...
            if is_current:
                ...
                _grad_v(hi, pygame.Rect(0, 0, w - 2, row_h), 0,
                        hi_c, hi_c, 35, 18)
                surf.blit(hi, (1, ry))
            ...
            _player_dot(surf, COL_DOT + dot_r, ry + row_h // 2, dot_r, PLAYER_COLORS[pid])
        ...
        _grad_v(glass, pygame.Rect(0, 0, w, glass_h), 0,
...
class MessageBar:
    """Barra messaggi bottom con animazione slide-up e fade-out."""
    ...
    def draw(self, screen, sw, sh):
        ...
        pygame.draw.rect(surf, (200, 200, 220, 200),
                        pygame.Rect(0, r, 4, h - r * 2))
...
class Toolbar:
    """Toolbar top-right con pulsanti."""
    ...
    def draw(self, screen, sw):
        ...
        _panel(panel, pygame.Rect(0, 0, panel_w, panel_h), 10,
               border_color=(60, 60, 80), border_w=1)
        ...
        for i, btn in enumerate(self._buttons):
            ...
            _panel(btn_surf, pygame.Rect(0, 0, bs, bs), 8,
                   c_top=_adj(c, +15), c_bot=_adj(c, -15),
                   a_top=240, a_bot=240,
                   border_color=(0, 0, 0), border_w=1)

            glass = pygame.Surface((bs, bs // 2), pygame.SRCALPHA)
            _grad_v(glass, pygame.Rect(0, 0, bs, bs // 2), 8,
...
class SettingsPopup:
    ...
    def draw(self, screen, sw, sh):
        ...
        _panel(popup, pygame.Rect(0, 0, pw, ph), r,
        ...
        _grad_v(glass, pygame.Rect(0, 0, pw, glass_h), 0,
        ...
        for i, btn in enumerate(self._buttons):
            ...
            if is_pair:
                ...
                for col, b in enumerate([btn, next_btn]):
                    ...
                    _panel(bs, pygame.Rect(0, 0, half_w, bh), 10,
                           c_top=_adj(c, +12), c_bot=_adj(c, -8),
                           a_top=240, a_bot=235,
                           border_color=(60, 60, 80), border_w=1)
                    gh = pygame.Surface((half_w, bh // 2), pygame.SRCALPHA)
                    _grad_v(gh, pygame.Rect(0, 0, half_w, bh // 2), 10,
                ...
                _panel(bs, pygame.Rect(0, 0, b_w, bh), 10,
                       c_top=_adj(c, +12), c_bot=_adj(c, -8),
                       a_top=240, a_bot=235,
                       border_color=(60, 60, 80), border_w=1)
                gh = pygame.Surface((b_w, bh // 2), pygame.SRCALPHA)
                _grad_v(gh, pygame.Rect(0, 0, b_w, bh // 2), 10,
                        (255, 255, 255), (255, 255, 255), 28, 2)
                bs.blit(gh, (0, 0))
